...
class MetricsAnalyzer:
    ...
    def print_summary(self, repositories: List[Dict], output_md_filename: str = None):
        ...
        if output_md_filename:
            ...
            # Adicionar as imagens dos gráficos no arquivo .md
            with open(output_md_filename, "a", encoding="utf-8") as md_file:
                md_file.write("\n## Gráficos:\n")

                # RQ01
                md_file.write("### RQ01 - Idade dos Repositórios (Histograma)\n")
                md_file.write(f"![RQ01 Hist]({rq01_hist_path})\n")
                md_file.write("### RQ01 - Idade dos Repositórios (Box Plot)\n")
                md_file.write(f"![RQ01 Box]({rq01_box_path})\n")

                # RQ02
                md_file.write("### RQ02 - Pull Requests Aceitas (Histograma)\n")
                md_file.write(f"![RQ02 Hist]({rq02_hist_path})\n")
                md_file.write("### RQ02 - Pull Requests Aceitas (Box Plot)\n")
                md_file.write(f"![RQ02 Box]({rq02_box_path})\n")

                # RQ03
                md_file.write("### RQ03 - Releases (Histograma)\n")
                md_file.write(f"![RQ03 Hist]({rq03_hist_path})\n")
                md_file.write("### RQ03 - Releases (Box Plot)\n")
                md_file.write(f"![RQ03 Box]({rq03_box_path})\n")

                # RQ04
                md_file.write("### RQ04 - Dias Desde a Última Atualização (Histograma)\n")
                md_file.write(f"![RQ04 Hist]({rq04_hist_path})\n")
                md_file.write("### RQ04 - Dias Desde a Última Atualização (Box Plot)\n")
                md_file.write(f"![RQ04 Box]({rq04_box_path})\n")

                # RQ05
                md_file.write("### RQ05 - Linguagens Mais Populares (Barras)\n")
                md_file.write(f"![RQ05 Barras]({rq05_bar_path})\n")
                md_file.write("### RQ05 - Linguagens Mais Populares (Pizza)\n")
